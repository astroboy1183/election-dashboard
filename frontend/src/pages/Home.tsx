      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '2.5rem 2rem' }}>
        ...
        {summary && summary.total_mlas > 0 && (
          <div className="card" style={{ marginBottom: '1.5rem' }}>
            ...
            {/* Top parties chart + NOTA dissatisfaction + Postal Ballots block moved to /all-india
                to keep this Home card light and focused on the at-a-glance tiles only. */}
          </div>
        )}

        {/* CTA → All-India Analytics deep-dive */}
        <div
          role="button"
          tabIndex={0}
          onClick={() => navigate('/all-india')}
          onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); navigate('/all-india') } }}
          style={{
            marginBottom: '1.5rem',
            padding: '1rem 1.25rem',
            borderRadius: 12,
            background: 'linear-gradient(135deg, rgba(167,139,250,0.10) 0%, rgba(6,182,212,0.10) 100%)',
            border: '1px solid rgba(167,139,250,0.35)',
            cursor: 'pointer',
            display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12,
            transition: 'transform 0.15s, background 0.15s',
          }}
          title="Open the All-India Analytics deep-dive"
        >
          <div>
            <div style={{ fontSize: '1rem', fontWeight: 800, color: 'var(--text-primary)', marginBottom: 2 }}>
              📊 All-India Analytics — deeper cross-state insights
            </div>
            <div style={{ fontSize: '0.82rem', color: 'var(--text-secondary)' }}>
              Top parties by MLA count · NOTA-decided seats · postal ballot leaders &amp; 2021→2026 swing
            </div>
          </div>
          <div style={{
